...
import type { PayableBalance, PaymentMethod } from "../../lib/types";
...
export function RecordPurchasePaymentModal({ payable, onSubmit, onClose }: Props) {
  const [amount, setAmount] = useState(payable.balance.toString());
  ...
  const valid = amountNum > 0 && amountNum <= payable.balance + 0.01;
      ...
      <div className="w-full max-w-md rounded-lg border border-(--border-color) bg-(--bg-primary) p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-(--text-primary)">Registrar pago — {payable.purchase_number || "orden"}</h2>
          </div>
        ...
        <p className="mb-3 text-xs text-(--text-secondary)">
          Saldo pendiente: <span className="font-mono font-semibold text-(--text-primary)">{formatCOP.format(payable.balance)}</span>
        </p>
