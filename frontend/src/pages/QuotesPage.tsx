...
export function QuotesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return quotes.filter((q) => {
      ...
      if (search.trim()) {
        ...
        const matchesNumber = q.number?.toLowerCase().includes(q2);
        const matchesCustomer = customerName(q.customer_id).toLowerCase().includes(q2);
        if (!matchesNumber && !matchesCustomer) return false;
      }
      return true;
    });
  }, [quotes, status, search, customerName]);
  ...
  return (
    <div className="p-4">
      <Breadcrumbs items={[{ label: "Ventas", to: "/sales" }, { label: "Cotizaciones" }]} />
      <div className="mb-3 flex items-center justify-between">
        <h1 className="flex items-center gap-2 text-sm font-semibold text-(--text-primary)">
          <ClipboardList className="h-4 w-4 shrink-0 text-(--accent-primary)" />
          Cotizaciones
        </h1>
        <Button type="button" icon={<Plus className="h-3.5 w-3.5" />} onClick={() => navigate("/sales/quotes/new")}>
          Nueva cotización
        </Button>
      </div>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <select
          value={status}
          onChange={(e) => { setStatus(e.target.value as QuoteStatus | ""); setOffset(0); }}
      ...
      {quotes === null ? (
        ...
        <p className="text-xs text-(--text-secondary)">
          {hasFilters ? "No hay cotizaciones que coincidan con los filtros." : "Todavía no has creado ninguna cotización."}
        </p>
      ) : (
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-(--bg-tertiary) text-(--text-secondary)">
              <tr>
                ...
                <th className="px-3 py-2 font-medium">Válida hasta</th>
            ...
            <tbody>
              {page.map((q, i) => {
                const total = q.lines.reduce((s, l) => s + l.total, 0);
                return (
                  <tr
                    key={q.id}
