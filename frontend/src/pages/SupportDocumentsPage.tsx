...
export function SupportDocumentsPage() {
  ...
  useEffect(() => {
    setLoadingPage(true);
    listDocuments({
      dian_document_type_code: SUPPORT_DOCUMENT_DIAN_TYPE,
      limit: PAGE_SIZE + 1,
      offset,
      ...(status && { status }),
      ...(from && { from }),
      ...
      .then(setDocuments)
      .catch((err) => setError(err instanceof ApiError ? err.message : "No se pudieron cargar los documentos soporte"))
  ...
  const hasRefs = page?.some((d) => (d.na_count ?? 0) > 0) ?? false;
  ...
  return (
    <div className="p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <Button type="button" icon={<Plus className="h-3.5 w-3.5" />} onClick={() => navigate("/documents/support-documents/new")} style={{ backgroundColor: DOC_COLORS["/documents/support-documents"] }}>
      ...
      {page === null ? (
        ...
        <p className="text-xs text-(--text-secondary)">
          {hasFilters ? "No hay documentos soporte que coincidan con los filtros." : "Todavía no has creado ningún documento soporte."}
        </p>
      ) : (
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-(--bg-tertiary) text-(--text-secondary)">
              <tr>
                ...
                {hasRefs && <th className="px-3 py-2 font-medium">Referencia</th>}
              ...
              {page.map((d, i) => {
                ...
                return (
                  <tr
                    ...
                    onClick={() => navigate(`/documents/support-documents/${d.id}`)}
                  >
                    ...
                    {hasRefs && (
                      <td className="px-3 py-2">
                        {(d.na_count ?? 0) > 0 && (
                          <span translate="no" className="rounded px-1.5 py-0.5 text-[10px] font-medium bg-(--color-warning-bg) text-(--color-warning-text)">NA</span>
                        )}
                      </td>
                    )}
