...
-- Retenciones, activos fijos, IVA (F300), conciliación de cartera, forex y presupuesto.
--
-- Estado FINAL de fixed_assets: incluye source_document_id/type para trazabilidad al DS/FE
-- de compra que originó el activo.

-- ── Retenciones ───────────────────────────────────────────────────────────────────────────────
--
-- rate_bp: tarifa en basis points (1 bp = 0.01%; ej. 250 = 2.5%).
-- min_base_uvt: base mínima en UVT por debajo de la cual NO se practica retención (0 = siempre).
...
-- Valor del UVT (Unidad de Valor Tributario) por año, en centavos. Publicado anualmente por la DIAN.
...
-- ── Activos fijos (PPE) ───────────────────────────────────────────────────────────────────────
--
-- asset_account        → cuenta del activo (1504/1520/1524…)
-- depreciation_account → gasto de depreciación (516005/516010…)
-- accumulated_account  → depreciación acumulada (159205/159220…)
-- source_document_id/type: traza al DS/FE de compra que originó el activo (nullable).
...
-- Índice parcial: impide depreciar el mismo período dos veces.
...
--
-- Empareja una línea débito (factura/cargo) con su línea crédito (pago/abono).
...
-- from_currency/to_currency sin FK a catálogos: mismo patrón de aislamiento de módulos
-- que ica_tariffs.municipality_code — el módulo de forex es autónomo.
