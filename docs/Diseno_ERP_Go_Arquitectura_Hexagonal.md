...
- **RBAC — implementado en versión jerárquica simple, no granular por módulo.** El rol (`owner`/`admin`/`member`) viaja en el JWT (`security.user_companies` → login/select-company lo resuelven) y `shared/tenant.CanManage(ctx)` protege server-side las operaciones administrativas: invitar usuarios, y perfil/credenciales/logo/configuración de empresa. `member` opera el resto del ERP sin restricción — pero **no hay granularidad por módulo** (el `counter`-solo-ve-contabilidad / `user`-solo-ve-ventas del diseño de abajo no se construyó; eso sigue siendo aspiracional).
  - ✅ **Resuelto (sesión 2026-08-07):** se auditaron los 10 endpoints protegidos con `requireManage`/`CanManage` en todo el backend (security, company, accounting, purchase, sales) y se mapeó cada uno a su botón real en el frontend — de los 10, solo 8 (los de `company`: perfil, credenciales/certificado, logo, color de marca) tienen UI hoy; `handleInvite` y los 3 `handleSetNumberCounter`/`handleSetVoucherCounter` no tienen ningún botón todavía, nada que ocultar ahí. Nuevo hook `useCanManage()` (deriva de `user.role`, ya viajaba en login/`/auth/me`) + componente `ManageOnlyHint` para el texto explicativo, centralizados para no desincronizarse entre los 4 componentes de `Configuración → Empresa` (`CompanyProfileForm`, `SoftwareCertificateForm`, `LogoForm`, `BrandColorForm`). Los 3 primeros usan `<fieldset disabled={!canManage} className="contents">` (deshabilita todo lo de adentro sin afectar el layout); `CompanyProfileForm` oculta directamente el botón "Editar" (no hace falta fieldset porque el formulario ni siquiera se monta hasta que se hace clic ahí). Deliberadamente sin tocar `NumberingRangesPanel` ni `PublicRegistrationPanel` — ninguno de los dos llama un endpoint protegido por `CanManage` en el backend hoy, así que restringirlos en el frontend habría sido más estricto que la política real del backend.
