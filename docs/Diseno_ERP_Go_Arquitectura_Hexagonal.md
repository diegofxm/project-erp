...
- **RBAC — implementado en versión jerárquica simple, no granular por módulo.** El rol (`owner`/`admin`/`member`) viaja en el JWT (`security.user_companies` → login/select-company lo resuelven) y `shared/tenant.CanManage(ctx)` protege server-side las operaciones administrativas: invitar usuarios, y perfil/credenciales/logo/configuración de empresa. `member` opera el resto del ERP sin restricción — pero **no hay granularidad por módulo** (el `counter`-solo-ve-contabilidad / `user`-solo-ve-ventas del diseño de abajo no se construyó; eso sigue siendo aspiracional).
  - ✅ **Resuelto (sesión 2026-08-07):** se auditaron los 10 endpoints protegidos con `requireManage`/`CanManage` en todo el backend (security, company, accounting, purchase, sales) y se mapeó cada uno a su botón real en el frontend — de los 10, solo 8 (los de `company`: perfil, credenciales/certificado, logo, color de marca) tienen UI hoy; `handleInvite` y los 3 `handleSetNumberCounter`/`handleSetVoucherCounter` no tienen ningún botón todavía, nada que ocultar ahí. Nuevo hook `useCanManage()` (deriva de `user.role`, ya viajaba en login/`/auth/me`) + componente `ManageOnlyHint` para el texto explicativo, centralizados para no desincronizarse entre los 4 componentes de `Configuración → Empresa` (`CompanyProfileForm`, `SoftwareCertificateForm`, `LogoForm`, `BrandColorForm`).
    - **Primer intento (descartado tras probarlo en vivo): `<fieldset disabled={!canManage} className="contents">`.** No bloqueaba de verdad — `display: contents` en un `<fieldset>` tiene un bug conocido en Chrome/Edge donde el navegador pierde el rastro de qué elementos son descendientes del fieldset (al no generar caja propia), así que el `disabled` no se propaga de forma confiable. Además, mostrar el control habilitado-pero-atenuado con una nota es un patrón de upsell de SaaS, no el que usan ERPs serios (Odoo, SIESA): ahí quien no tiene permiso directamente **no ve** el control de edición.
    - **Diseño final: ocultar el control por completo, no deshabilitarlo.** `CompanyProfileForm` oculta el botón "Editar" (el formulario ni se monta hasta hacer clic ahí). `LogoForm`/`BrandColorForm` muestran una vista de solo lectura (logo o swatch de color, sin inputs ni botones) cuando `!canManage`. `SoftwareCertificateForm` — sus 3 secciones (Software DIAN, Software NE, Certificado) ya tenían una vista "configurado" de solo lectura; se le quitó el botón "Eliminar" a esa vista para quien no puede gestionar, y cuando NO está configurado se reemplaza el formulario de alta por un mensaje ("Sin configurar — solo un administrador o dueño puede configurarlo.") en vez de dejarlo vacío e interactuable.
    - Deliberadamente sin tocar `NumberingRangesPanel` ni `PublicRegistrationPanel` — ninguno de los dos llama un endpoint protegido por `CanManage` en el backend hoy, así que restringirlos en el frontend habría sido más estricto que la política real del backend.
    - **Causa raíz real (encontrada al probar con un `member` de verdad): `user.role` en las respuestas de auth NUNCA fue el rol por empresa.** El ocultamiento de arriba no bloqueaba nada porque el dato que `useCanManage()` lee siempre decía `"admin"`, sin importar el rol real. `respondAuth()` (login/register/select-company/accept-invite) armaba `"role": result.User.Role` — el campo `Role` de la entidad `User` es solo un **default de alta** (`RegisterUseCase`/`InviteUserUseCase` lo fijan siempre en `"admin"`, sin excepción) y nunca representó el rol dentro de una empresa; el rol real solo vivía en `security.user_companies.role`, resuelto vía `GetRole(ctx, userID, companyID)` y usado *solo* para el claim `rol` del JWT — nunca se copiaba de vuelta al `AuthResult` que arma la respuesta JSON. Igual pasaba en `safeUser()` (`/auth/me`, `handleUpdateProfile`). Se agregó `domain.AuthResult.Role` (el rol ya resuelto, el mismo que va al JWT) y se usa ese en vez de `User.Role` en ambos puntos — `safeUser` ahora recibe el rol como parámetro (`tenant.GetRole(ctx)`, decodificado del JWT por el middleware). Verificado en vivo: login/`/auth/me` de un `member` real ahora sí devuelven `"role":"member"`.
...
**Corrección (sesión 2026-08-06): la empresa de un superadmin nace con el plan Interno.** Antes, ninguna empresa nueva quedaba con ningún plan asignado — ni siquiera la del propio superadmin (la empresa operadora de la plataforma), que dependía de que alguien entrara luego a `/admin/company` a asignárselo a mano. `company.CreateUseCase.Execute` ahora revisa si `creatorID` es superadmin (`MembershipLinker.IsSuperAdmin`, nuevo — consulta puntual a `security.users`) y si lo es, le asigna el plan `interno` de una vez (`PlanAssigner.AssignInternalPlan`, implementado en `company/infrastructure/saas.Adapter`, que busca el plan por `code="interno"` sembrado y llama `saas.SubscriptionUseCase.Assign`). Empresas creadas por un dueño normal (no superadmin) siguen sin plan hasta que el superadmin se lo asigne manualmente — es el comportamiento esperado, solo el caso del superadmin estaba incompleto. Verificado en vivo: empresa creada por un superadmin → `GET /saas/my-plan` trae `"plan_name":"Interno"` con los 3 módulos e ilimitado; empresa creada por un usuario normal → sigue en 404 sin suscripción.
