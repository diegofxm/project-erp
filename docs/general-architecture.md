...
Comunicación inicial entre servicios: **REST HTTP**.
A futuro: bus de eventos (NATS / RabbitMQ) para desacoplar completamente.
...
journal_entries (
  ...
  source,   -- "apidian", "payroll", "manual", etc.
...
accounting.Mapper.FromInvoice(doc) → JournalEntryRequest
        ↓
accounting.Client.PostJournal(entry)
        ↓
accounting-core API
        ↓
...
## 3. Lo que NO se toca ni reorganiza
...
## 4. Riesgos identificados y decisiones

### Consistencia transaccional

Si `apidian` confirma una FE pero el `accounting-core` está caído, la factura queda sin asiento.
**Decisión para MVP:** loggear el error y continuar — la FE ya fue a la DIAN y no se puede
revertir. Implementar reintento automático en Fase 2.

### Base de datos

**Decisión para MVP:** PostgreSQL compartido con schema separado (`accounting.*` vs `public.*`).
Migrar a BD independiente cuando haya razón operacional real (no antes).

### Posting rules

**Decisión para MVP:** reglas hardcodeadas en `accounting/mapper.go` de cada módulo.
...
### Fase 1 — Núcleo mínimo (accounting-core)
...
### Fase 2 — Adaptador en apidian
- [ ] `apidian/internal/integrations/accounting/`
- [ ] Mapper para FE y DS (posting rules básicas)
- [ ] Llamada al core al confirmar documento

### Fase 3 — Reportes completos
- [ ] Libro diario, libro mayor, estado de resultados, balance general
- [ ] UI en frontend: módulo `accounting/`

### Fase 4 — Módulos futuros
- [ ] `payroll/` con su propio adaptador contable
- [ ] `hr/` (empleados, contratos)
- [ ] `inventory/` (futuro lejano)

---

## 6. Relación entre módulos (visión final)

```
                        ERP

          |              |              |
       Apidian        Payroll       Inventory
          |              |              |
...
*Documento generado: 2026-07-24. Actualizar cuando cambien decisiones de arquitectura.*
