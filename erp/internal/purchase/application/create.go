...
// 2026-08-11 -- crear una orden directamente, sin pasar por cotización, siempre fallaba).
...
func (uc *CreateUseCase) Execute(ctx context.Context, companyID uuid.UUID, req CreateRequest) (*domain.PurchaseOrder, error) {
	...
	seq, err := uc.repo.NextPurchaseNumber(ctx, companyID, issueDate.Year())
	if err != nil {
	...
	o := domain.PurchaseOrder{
		...
		Number:       fmt.Sprintf("OC-%d-%05d", issueDate.Year(), seq),
