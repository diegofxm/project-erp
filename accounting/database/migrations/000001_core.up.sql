...
--
-- Estado FINAL de journal_entries: voucher_type/voucher_number, source_document_id/type, book.
-- Estado FINAL de journal_lines: BIGINT centavos, third_party_nit, foreign_amount/currency.
-- Estado FINAL de bank_statement_lines: BIGINT centavos.
-- (La historia de ALTERs queda en el git — estas tablas nacen con el esquema definitivo.)
...
-- ── Asientos contables ────────────────────────────────────────────────────────────────────────
--
-- voucher_type/voucher_number: comprobante asignado (CE, CI, NC, NI…).
-- source_document_id/type: trazabilidad a documentos fuente (FE, DS, NA, NC, ND, nómina…).
--   Nullable: asientos manuales, de cierre, apertura y depreciación no tienen fuente.
-- book: doble libro PCGA (Decreto 2649) / NIIF (IFRS). 'BOTH' = aplica a ambos (la mayoría).
...
-- Líneas de asiento: montos en BIGINT centavos (no NUMERIC — nunca float en contabilidad).
-- third_party_nit: NIT del tercero. Obligatorio en Colombia para Medios Magnéticos / Exógena.
--   Nullable: asientos de apertura/cierre y ajustes internos no tienen tercero.
-- foreign_amount/foreign_currency: monto en moneda extranjera antes de convertir a COP.
--   Nullable: asientos en COP no lo usan.
...
--
-- Los tipos de sistema (CJ, AP) no necesitan registro previo — el contador funciona
-- independientemente de esta tabla.
...
-- Extracto bancario: montos en BIGINT centavos (igual que journal_lines).
