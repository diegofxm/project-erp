...
import (
	...
	"github.com/diegofxm/api-dian/internal/config"
	"github.com/diegofxm/api-dian/internal/database"
)

// Server is the HTTP entry point for the api-dian service.
type Server struct {
	cfg        *config.Config
	log        *zap.Logger
	db         *database.DB
...
func (s *Server) routes() http.Handler {
	...
	if s.db != nil {
		mux.Handle("/api/", api.New(s.log, s.db, s.cfg.IssuerSecretsKey).Handler())
	}
