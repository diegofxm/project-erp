...
A Documento Equivalente Electrónico (e.g. a POS ticket) is `builder.BuildInvoice` and `cude.Compute` again — same `domain.Invoice`, same pipeline as above — with four fields set differently: `ProfileID: "DIAN 2.1: Documento Equivalente POS"`, `CustomizationID: "10"`, `DocumentTypeCode: "20"` (the Technical Annex Documento Equivalente Electrónico V1.0, section 16.3, lists the codes for the other 9 sub-types), and `softwarePIN` passed to `cude.Compute` where the invoice above passes `technicalKey` to `cufe.Compute`. Its Adjustment Notes are `builder.BuildCreditNote`/`BuildDebitNote` with `DocumentTypeCode`/`CreditNoteTypeCode` `"94"`/`"93"` and `CustomizationID` set to the *referenced* document's own type code (e.g. `"20"` for one adjusting a POS ticket).
